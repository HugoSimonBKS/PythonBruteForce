...
def paquet(txt, paq=1) :
	chars = list(txt.upper())
	print(chars)
	crypt = []
	nbcharparpaq=0
	for i in range(ceil(len(chars)/paq)): 	#on se deplace paq par paq
		nbcharparpaq=0
		temp = ""
		for j in range(i*paq, i*paq+paq):			# on va du debut d'un paquet a la fin de celui ci
			if(j >= len(chars)):
				break
			if(j > i*paq and j < i*paq+paq and codex(chars[j]) < 10):
				temp += "0"			# si le char est a un seul chiffre et n'est pas le premier on fill avec des 0
	...
	if(nbcharparpaq < paq):
		michelSardou = ""
		for osef in range (len(chars)-paq+nbcharparpaq, len(chars)):
			michelSardou += "00"
		crypt[len(crypt)-1] += michelSardou
	return crypt
...
def PGCD(a,b) :
	if a == 0:
		...
		g, v, u = PGCD(b % a, a)
		return (g, u - (b // a) * v, v) #g = pgcd,  (b // a) = quotient, u - (b // a) * v = new v et u devient v
...
print(inv_mod(2365, 382))
print(paquet("tamerlaputedebeauvais", 4))
#########################
#	Partie 1 : CESAR	#
#########################
